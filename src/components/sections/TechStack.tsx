const stackRows = [
  { comp: 'Broker + datos en tiempo real', tech: 'Alpaca API',                       techColor: '#e8a400', cost: '$0',          costType: 'free' },
  { comp: 'Datos históricos ML',           tech: 'Yahoo Finance (yfinance)',          techColor: '#7aadff', cost: '$0',          costType: 'free' },
  ...
  { comp: 'Frontend',                      tech: 'React + Tailwind CSS',              techColor: '#e8a400', cost: '$0',          costType: 'free' },
  { comp: 'Deploy frontend',               tech: 'Vercel + GitHub',                  techColor: '#7aadff', cost: '$0',          costType: 'free' },
  { comp: 'Backend API',                   tech: 'FastAPI (Python)',                  techColor: '#e8a400', cost: '~$5/mes',     costType: 'paid' },
  ...
  { comp: 'IDE principal',                 tech: 'Claude Code',                       techColor: '#e8a400', cost: '~$100/mes',   costType: 'paid' },
...
const dbTables = [
  { name: 'portfolio',       desc: 'Posiciones actuales',       color: '#e8a400' },
  { name: 'movements',       desc: 'Historial de transacciones', color: '#7aadff' },
  ...
  { name: 'performance',     desc: 'Ganancias y métricas',      color: '#f5bc2e' },
]

export default function TechStack() {
  return (
    <section id="stack" className="py-24" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      <div className="max-w-5xl mx-auto px-9">
        <div className="section-tag">04 — Stack tecnológico</div>
        <h2 className="font-cond font-semibold mb-9" style={{ fontSize: 'clamp(26px,3.5vw,42px)' }}>
          Tecnologías del proyecto
        </h2>

        {/* Stack table */}
        <div className="rounded-[14px] overflow-hidden" style={{ background: '#101929', border: '1px solid rgba(255,255,255,0.06)' }}>
          {/* Header */}
          <div className="grid gap-4 px-6 py-3.5" style={{ gridTemplateColumns: '200px 1fr 100px', background: '#16233a', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          {stackRows.map((r, i) => (
            <div key={r.comp}
              ...
              style={{
                ...
                borderBottom: i < stackRows.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none',
              }}
              onMouseEnter={e => (e.currentTarget.style.background = '#16233a')}
              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
              <span className="text-u-muted text-[12px]">{r.comp}</span>
              <span className="font-medium" style={{ color: r.techColor || '#f0f4fa' }}>{r.tech}</span>
        ...
        {/* Supabase tables */}
        <div className="mt-7">
          <h3 className="text-[17px] font-semibold mb-3.5">Tablas principales — Supabase</h3>
